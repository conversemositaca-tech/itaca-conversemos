...
salen por la instancia conversemoslima".
    ...
    python manage.py registrar_instancia_evolution --sede lima  --instancia conversemoslima  --responsable Ayvi
    python manage.py registrar_instancia_evolution --sede piura --instancia conversemospiura --responsable Yazmín
    python manage.py registrar_instancia_evolution --sede lima  --instancia conversemoslima  --desactivar
    python manage.py registrar_instancia_evolution --instancia vibery --entorno prueba --responsable Desarrollo
    python manage.py registrar_instancia_evolution --listar

Una instancia `--entorno prueba` sirve para validar el circuito (envío real,
id externo, historial) sin que NUNCA se elija para escribirle a un paciente:
`instancia_para` solo devuelve líneas oficiales.
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--entorno", default="oficial", choices=["oficial", "prueba"],
                            help="'prueba' = ambiente de pruebas: nunca le escribe a un paciente.")
        parser.add_argument("--responsable", default="",
                            help="Quién atiende esa línea (ej. Ayvi, Yazmín).")
    ...
    def _listar(self, clinica):
        ...
        for i in filas:
            ...
            etiqueta = "PRUEBAS" if not i.es_oficial else (i.sede or "(sin sede)")
            quien = f" · {i.responsable}" if i.responsable else ""
            self.stdout.write(
                f"  · {etiqueta:<8} → {i.nombre_instancia:<24} [{estado}]{quien} "
    ...
    def handle(self, *args, **options):
        ...
        entorno = options["entorno"]
        responsable = (options["responsable"] or "").strip()[:120]
        if not nombre:
            raise CommandError("Falta --instancia (o usa --listar).")
        if entorno == "oficial" and not sede:
            raise CommandError("Falta --sede (lima, piura o ambas). Para una línea de "
                               "pruebas usa --entorno prueba.")

        activo = not options["desactivar"]
        # Solo puede haber UNA línea OFICIAL activa por sede (lo sostiene un
        # constraint): si ya hay otra, se apaga antes para no chocar contra la base.
        if activo and entorno == "oficial" and sede in ("lima", "piura"):
            otras = (InstanciaEvolution.objects
                     .filter(clinica=clinica, sede=sede, activo=True, entorno="oficial")
        ...
        defaults = {"sede": sede, "activo": activo, "entorno": entorno}
        if responsable:
            defaults["responsable"] = responsable
        instancia, creada = InstanciaEvolution.objects.update_or_create(
            clinica=clinica, nombre_instancia=nombre, defaults=defaults,
        )
        verbo = "Registrada" if creada else "Actualizada"
        estado = "activa" if instancia.activo else "apagada"
        donde = "PRUEBAS" if entorno == "prueba" else sede
        self.stdout.write(self.style.SUCCESS(
            f"{verbo}: {donde} → {nombre} ({estado})"
            + (f" · responsable: {instancia.responsable}" if instancia.responsable else "") + "."))
        if entorno == "prueba":
            self.stdout.write("Es una línea de PRUEBAS: nunca se elegirá para escribirle a un paciente.")
        self.stdout.write(
